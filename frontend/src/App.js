...
const navItems = [
  ...
  { path: "/my-documents", label: "My Documents", icon: FolderOpen, description: "Your saved PKMLs" },
...
function App() {
  ...
  const [pkmlContent, setPkmlContent] = useState(() => {
    return localStorage.getItem("pkml-draft") || "";
  });
  // Track which saved document is currently being edited (for Save to update, not create)
  const [activeDocId, setActiveDocId] = useState(() => localStorage.getItem("pkml-doc-id") || null);
  const [activeDocSlug, setActiveDocSlug] = useState(() => localStorage.getItem("pkml-doc-slug") || null);
  ...
  // Load content from Gallery/Registry fork
  const handleLoadTemplate = (content) => {
    setPkmlContent(typeof content === "string" ? content : JSON.stringify(content, null, 2));
    // Clear active doc — forked content is a new document
    setActiveDocId(null);
    setActiveDocSlug(null);
    localStorage.removeItem("pkml-doc-id");
    localStorage.removeItem("pkml-doc-slug");
  };

  // Load a saved document for editing (from My Documents)
  const handleLoadDocument = (content, docId, docSlug) => {
    setPkmlContent(content);
    setActiveDocId(docId);
    setActiveDocSlug(docSlug);
    if (docId) localStorage.setItem("pkml-doc-id", docId);
    if (docSlug) localStorage.setItem("pkml-doc-slug", docSlug);
  };

  // Called by EditorPage when a save/publish succeeds
  const handleDocSaved = (docId, docSlug) => {
    setActiveDocId(docId);
    setActiveDocSlug(docSlug);
    if (docId) localStorage.setItem("pkml-doc-id", docId);
    if (docSlug) localStorage.setItem("pkml-doc-slug", docSlug);
          ...
          <main className="flex-1 overflow-hidden" data-testid="main-content">
            <Routes>
              ...
              <Route path="/builder" element={
                <BuilderPage content={pkmlContent} onContentChange={setPkmlContent} />
              } />
              ...
              <Route path="/my-documents" element={
                <MyDocumentsPage onLoadDocument={handleLoadDocument} />
              } />
              <Route path="/registry" element={
                <RegistryPage onForkTemplate={handleLoadTemplate} />
              } />
              <Route path="/view/:slug" element={
                <ViewPage onFork={handleLoadTemplate} />
              } />
            </Routes>
          </main>
        </div>
