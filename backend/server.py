...
def parse_readme_to_pkml(readme: str) -> dict:
    """Parse README.md content and generate draft PKML"""
    lines = readme.split('\n')
    ...
    # Extract tagline and description from first paragraphs
    in_features = False
    ...
    for i, line in enumerate(lines):
        ...
        # Get tagline from first non-header line
        if not stripped.startswith('#') and stripped and not tagline:
            tagline = stripped[:100]
            continue
        ...
        if in_features:
            if stripped.startswith('- ') or stripped.startswith('* '):
                ...
                # New section, stop extracting features
                if not stripped.lower().replace('#', '').strip().startswith('feat'):
                    in_features = False
    ...
    return {
        ...
        "product": {
            "name": name,
            "tagline": tagline if tagline != "Product description" else f"{name} - Your product description here",
        ...
        "features": features if features else [
            {
                "id": "feat_1",
                "name": "Core Feature",
                "description": "Describe your main feature here",
                "user_benefit": "What users gain from this feature",
                "priority": "primary"
            }
        ]
    }
...
async def parse_readme(request: ReadmeImportRequest):
    """Import README.md and generate draft PKML"""
    pkml = parse_readme_to_pkml(request.readme_content)
    return {"pkml": pkml}
